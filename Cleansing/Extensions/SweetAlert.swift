...
open class SweetAlert: UIViewController {
    ...
    fileprivate func setupContentView() {
        ...
        contentView.layer.borderColor = UIColor(hexString: "004080").cgColor//UIColor.colorFromRGB(0xCCCCCC).cgColor
    ...
    fileprivate func resizeAndRelayout() {
        ...
        if self.subTitleTextView.text.isEmpty == false {
            ...
            let rect = subtitleString.boundingRect(with: CGSize(width: width, height: 0.0), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font:UIFont(name: kFont, size: 20)], context: nil)
            textViewHeight = ceil(rect.size.height) + 60.0
        ...
        for button in buttons {
            ...
            buttonRect.append(string.boundingRect(with: CGSize(width: width, height:0.0), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes:[NSAttributedString.Key.font:UIFont(name: kFont, size: 20)], context:nil))
        }
    ...
    open func showAlert(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
            self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
                otherButtonTitle,otherButtonColor: UIColor(hexString: "004080"))
            userAction = action
            return self
    }
