...
class AdminDetailTimeCardVC: UIViewController {
...
//        organizationName.setTitle("Select a team members", for: .normal)
...
//       if projectType == 2{
//        _ = SweetAlert().showAlert("", subTitle:  "Approved time cards cannot be edited", style: AlertStyle.warning,buttonTitle:"OK".localizeString(string: lang))
//    }else{
...
//            _ = SweetAlert().showAlert("", subTitle:  "Please contact admin to update your project/task.", style: AlertStyle.warning,buttonTitle:"OK".localizeString(string: lang))
...
extension AdminDetailTimeCardVC {
    ...
    func firstCall()
    {
        ...
        if let loadedArray = UserDefaults.standard.array(forKey: "spt") as? [Any] {
            ...
            projectSelect.setTitle( loadedArray[2] as? String ?? "Select project & task".localizeString(string: lang) , for: .normal)
        ...
        saveBtn.setTitle("Approve & Save Changes".localizeString(string: lang), for: .normal)
...
extension AdminDetailTimeCardVC: UITableViewDelegate, UITableViewDataSource{
...
//                    } else {
//                        cell.radioBtn.setImage(UIImage(systemName: "square"), for: .normal)
//                        if let index = self.sm.firstIndex(of: showA[indexPath.row].nameCA) {
//                            self.sm.remove(at: index)
//                        }
//                        if let index = self.si.firstIndex(of: "\(showA[indexPath.row].idCA)") {
...
//                }
                ...
                if indexPath.row == selectedRow {
                    ...
                    memberId = "\(showA[selectedRow ?? 0].idCA)"
                }
...
extension AdminDetailTimeCardVC {
    ...
    func saveTimeCard(startTime : String, finishTime: String, project_id: Int, task_id: Int, breaks: [String], timecard_id: Int)
    {
        ...
        print("Access Token: \(accessToken)")
        var param: Parameters = ["":""]
        param = ["start_time": formattedDate1, "end_time": formattedDate2, "project_id": project_id, "task_id": task_id, "breaks": "", "work_id": timecard_id, "member_id": memberId, "managerId": managerId]
        print(param)
        AF.request(url, method: .post, parameters: param, encoding: URLEncoding.default, headers: ["Authorization": "Bearer "+accessToken+""])
            .validate(statusCode: 200..<300)
            .validate(contentType: ["application/json"])
                    ...
                    if self.status == 200
                    {
                        if let jsonData = try? JSONSerialization.data(withJSONObject: value),
                           let loginResponse = try? JSONDecoder().decode(TimeCardDetailsInfo.self, from: jsonData) {
                            self.coun = false
                            self.projectSelect.setTitle(loginResponse.timeCards[0].task_name  == "" ? "Select project & task".localizeString(string: lang) : loginResponse.timeCards[0].task_name, for: .normal)
...
//                            }
                            ...
                            self.organizationName.setTitle(loginResponse.timeCards[0].member_name, for: .normal)
                            self.memberId = "\(loginResponse.timeCards[0].member_id)"
                            self.timeAprover.setTitle(loginResponse.timeCards[0].manager_name, for: .normal)
                            self.managerId = "\(loginResponse.timeCards[0].manager_id)"
                        ...
                        if let jsonData = try? JSONSerialization.data(withJSONObject: value),
                           let loginResponse = try? JSONDecoder().decode(MembersResponse.self, from: jsonData) {
                            ...
                            for i in 0..<loginResponse.members.count{
                                self.showA.append(showCADetails.init(isSelected: false, nameCA: loginResponse.members[i].fullname, idCA: loginResponse.members[i].id))
                            }
                            progressHUD.hide()
                            self.approvOrganizTV.delegate = self
                            self.approvOrganizTV.dataSource = self
...
extension AdminDetailTimeCardVC: UITextFieldDelegate {
