//
//  AddAdminTimeCardVC.swift
//  Cleansing
//
//  Created by uis on 05/12/23.
//

import UIKit
import Alamofire
class AddAdminTimeCardVC: UIViewController {
    ...
    @IBAction func selectBtnTap(_ sender: UIButton) {
        if type == "A"{
            ...
            approverOrganizationView.isHidden = true
            let commaSeparatedString = sm.joined(separator: ", ")
            let commaSeparatedId = si.joined(separator: ", ")
            organizationName.setTitle(commaSeparatedString, for: .normal)
            memeberId = commaSeparatedId
        }
    }
...
extension AddAdminTimeCardVC: UITableViewDelegate, UITableViewDataSource{
...
extension AddAdminTimeCardVC {
    ...
    func getTaskTimer(startTime : String, finishTime: String, break_start: String, break_end: String, index: IndexPath)
    {
        ...
        //check for break end when adding seconfd first break time
    ...
    //MARK: Add Time Card
    func addTimeCard(startTime : String, finishTime: String, project_id: Int, task_id: Int, breaks: [String], member_id: String)
    {
        var formattedDate1 = ""
        var formattedDate2 = ""
        if reachability.isConnectedToNetwork() == false{
    ...
    func updateTimer() {
        if let startTime = startTimes, let finishTime = finishTimes {
            ...
            if hours > 0 {
                ...
                hourMinuteLabel.text = "\(minuteString)M"
            } else {
                hourMinuteLabel.text = "0H 0M"
            }
